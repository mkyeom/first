...
def home_page():
    ...
    _, col2, _ = container.columns([1, 3, 1])
        
    with col2:
        st.title('Enter Face:smile:')
        st.write('') # 공백
        st.write('') # 공백
        st.write('') # 공백
        ...
        container = st.container()
        _, col12, _ = container.columns([1, 3, 1]) # 비율 조절

        with col12:
            
            if st.button("사용자 인증하기"):
                st.session_state.current_page = "사용자 인증하기"
...
def page1():
    ...
    with col2:
        ...
        st.markdown('1. 눈을 3회 깜박여주세요.')
        st.markdown('2. **:red["Success"]** 문구가 나오면 :white_check_mark: **인증하기**를 눌러주세요.')  
        st.markdown('3. 얼굴을 화면 중앙에 위치하고 촬영해주세요.')
        ...
        capture_button = st.button(':white_check_mark: 인증하기')
        ...
        if st.button(":x: 다시 시도하기"):
            st.session_state.current_page = "다시 시도하기"
            st.experimental_rerun()
...
PAGES = { '다시 시도하기': home_page,  "사용자 인증하기": page1}
                       

# 현재 페이지를 저장하고 처리하는 session_state 정의
